...
│   ├── query_rewriter.py      # 多轮对话 Query 改写（含实体幻觉后置校验）
...
│   └── eval_rewrite.py        # 多轮对话 Query 改写评测
├── data/
...
│   └── multiturn_eval.json    # 多轮 Query 改写评测集（7 条，含代词/省略/话题切换/幻觉陷阱）
...
`MAX_HISTORY = 10` 把历史喂给生成端的 LLM，但**检索端每次只拿当前 query**。follow-up 问题如 "他的血量呢"、"那几费"、"她的初始遗物"，BM25 没有实体名可锚定、向量空间也漂到无关区域——**Hit@K 直接归零**，用户体验就是"上一句还在聊铁甲战士，下一句就不认识他了"。

### 方案：历史感知的 Query 改写 + 实体幻觉后置校验

每轮检索前先跑一次 `rewrite_query(question, history, client, index)`：

1. **LLM 自判断**：Prompt 明确指示"如果 query 已独立，原样返回"，让模型决定是否改写——避免手写启发式规则漏判代词/省略。
2. **只喂最近 4 条消息**（2 轮）给 rewriter——代词指代通常只指最近一轮；塞太多历史反而让 rewriter 被早期话题误导。
3. **后置校验拦截幻觉**：rewriter 本身是 LLM 也会编造。提取重写 query 中的实体名（复用 `router._find_entities`），若出现了"原 query 和 history 里都没出现过"的实体，判定为幻觉并**回退到原 query**——宁可检索稍差也不要自信地检错实体。
4. **只影响检索路径**：重写后的 query 送进 router / HyDE / BM25 / vector / rerank；**生成端仍拿原 query**（LLM 有完整 history，无需改）。
...
|------|-------|
| 代词消解（他/她/它 → 实体名） | 3/3 |
| 省略主语补全（"是几费的" → "力量药水是几费的"） | 1/1 |
| 话题切换保留（新 query 含完整实体名，不污染） | 1/1 |
| 已独立 query 原样返回（检测 false positive 改写） | 1/1 |
| 幻觉陷阱（历史里没有有效实体时不能瞎塞） | 1/1 |
| **总通过率** | **7/7 (100%)** |

运行：`python scripts/eval_rewrite.py`。

### 代价和权衡

- **延迟 +1 轮 LLM 调用**（~500ms）。对 Follow-up 这是必须付出的成本，零检索召回更糟。
- **有 history 才触发**——首轮 query 零成本。
- **后置校验零 API**——纯字符串匹配 `index` 里的已知实体名。
